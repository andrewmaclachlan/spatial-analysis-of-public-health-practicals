...
If you look at the output you will see that the `from_id` column has changed from an LSOA to postcode. R5R has tried to help us here bu reducing the spatial size and snapping the origin to a lower spatial scale, however we need this to be an LSOA for mapping. So we must use a [lookup table](https://open-geography-portalx-ons.hub.arcgis.com/datasets/ons::postcode-to-oa-2021-to-lsoa-to-msoa-to-lad-february-2025-best-fit-lookup-in-the-uk/about) to convert it back

```{r}
Lookup <- st_read("prac5_data/PCD_OA21_LSOA21_MSOA21_LAD_FEB25_UK_LU.csv")

Lookup <- Lookup %>%
  mutate(pcd7 = str_replace_all(pcd7, " ", ""))

TTM_Oxford_walk_mapped <- TTM_Oxford_walk %>%
  left_join(Lookup %>% select(pcd7, lsoa21cd), 
            by = c("from_id" = "pcd7")) %>%
  rename(
    from_postcode = from_id,  # keep the original postcode in a new column
    from_id = lsoa21cd  # LSOA code becomes the new from_id
  )
```
...
TTM_Oxford_walk2 <- TTM_Oxford_walk_mapped %>%
...
remove and inf values as these won't map
